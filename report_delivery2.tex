...
\subsection{Bill}
A bill can be used to buy products at a PostOffice, as well as to pay for any kind of deliverable. A bill is described by its bill number, price (which is the sum of prices of the items of the delivery plus any other item bought at the PostOffice), issue time, issuing shopKeeper, the client that paid for it and the seller, which is the company to which the client is paying for a delivery.
...
\begin{center} \setlength{\tabcolsep}{1pt}
    \begin{tabular}{r p{144mm}}
        ...
        Address         & (\uline{idAddress}, (zipNumber,zipCountry)$\rightarrow$ZipCode, streetName, streetNumber, doorNumber, personName) \\
        ...
        Delivery        & (\uline{idDelivery}, from$\rightarrow$PAddress, to$\rightarrow$PAddress, registeredBy$\rightarrow$ShopKeeper, order$\rightarrow$Order, timeRegister, weight, category, service, payMethod) \\
                        & payMethod$\rightarrow$Bill or payMethod$\rightarrow$Service \\
        ...
        BillItem        & (\uline{bill}$\rightarrow$Bill, \uline{catalogItem}$\rightarrow$CatalogItem, priceThen, amount) \\
...
\subsubsection{ZipCode}
$\{\text{country},\text{number}\} \rightarrow \{\text{town, postman}\}$
\subsubsection{Address}
$\{\text{idAddress}\} \rightarrow \{\text{zipNumber}, \text{zipCountry}, \text{streetName}, \text{streetNumber}, \text{doorNumber}, \text{personName}\}$
